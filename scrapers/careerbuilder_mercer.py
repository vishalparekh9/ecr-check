...
# Token
token = 'CAREERBUILDER_MERCER'


class CRAWLER(object):
    def __init__(self):
        self.location = 'mercer'.replace(' ', '+')
        self.getHeaders = {
    ...
    def get_location(self):
        try:
            res = self.session.get(
                'https://www.careerbuilder.com/autocomplete/location/?term=' + str(self.location.replace('+', ' ')),
    ...
    def scrapDomain(self, company, domains):
        try:
            newUrl = str("https://www.careerbuilder.com/company/" + str(company).replace(" ",
                                                                                         "-").lower() + "/" + domains + ".js?co_overview=true")
        ...
        try:
            ...
            for link in links:
                ...
                try:
                    ...
                    jobObj['location'] = link.find("div", {"class": "data-details"}).find_all("span")[
                                             1].text.strip() + ', United States'

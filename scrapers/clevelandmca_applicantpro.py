...
# Token
token = 'APPLICANTPRO_CLEVELANDMCA_JOBS'


class CRAWLER(object):
    def __init__(self):
        self.baseUrl = 'https://clevelandymca.applicantpro.com/jobs/'
            ...
            if isloaded:
                ...
                if links:
                    for link in links:
                        ...
                        jobObj["location"] = link.find("ul", {"class": "listing-details"}).find("li").text.replace("\r",
                                                                                                                   "").replace(
                            "\n", "").strip()

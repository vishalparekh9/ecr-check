...
# Token
token = 'E3APPLICANTS_CPCBEHAVIORAL_JOBS'


class CRAWLER(object):
    def __init__(self):
        self.baseUrl = 'https://www.cpcbehavioral.org'
    ...
    def process_logic(self):
        try:
            url = f'https://cpcbehavioralhealthcare.e3applicants.com/careers/'
            print(url)
                ...
                if len(links) > 0:
                    for link in links:
                        try:
                            ...
                            url = "https://cpcbehavioralhealthcare.e3applicants.com" + str(link.find("a").get("href"))
                            jobObj['title'] = link.find("span", {"class": "title"}).text.replace("\r", "").replace("\n", "").replace("\t", "").strip()
                            if link.find("small") is not None:

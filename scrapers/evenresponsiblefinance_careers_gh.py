...
# Token
token = 'EVEN_CAREER_JOBS_GH'


class CRAWLER(object):
    def __init__(self):
        self.baseUrl = 'https://www.even.com/'
    ...
    def process_logic(self):
        try:
            url = 'https://boards-api.greenhouse.io/v1/boards/evenresponsiblefinance/jobs'
                ...
                if "jobs" in str(res.json()):
                    for data in res.json()["jobs"]:
                        if data:
                            ...
                            isloaded, res = self.get_request('https://boards.greenhouse.io/embed/job_app?for=evenresponsiblefinance&token='+str(url).split('/jobs/')[1])
                            if isloaded:

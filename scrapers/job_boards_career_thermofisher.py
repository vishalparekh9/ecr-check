...
# Token
token = 'JOB_BOARD_CAREER_THERMO_FISHER_JOBS'


class CRAWLER(object):
    def __init__(self):
        self.baseUrl = 'https://www.thermofisher.com'
    ...
    def process_logic(self):
        try:
            ...
            while isPage:
                ...
                url = f'https://jobs.thermofisher.com/global/en/search-results?from={page}&s=1'
                print(url)
                                ...
                                for rd in jsonRawData['jobs']:
                                    ...
                                    if isloaded:
                                        try:
                                            ...
                                            jd = jobData.find('input', {'id': 'preLoadJSON'}).get("value")
                                            jDesc = json.loads(str(jd))['Jobdetails']['JobDetailQuestions']
                                            for desc in jDesc:
                                                if len(desc['AnswerValue']) > 1000:
                                                    jobObj['description'] = str(desc['AnswerValue'])

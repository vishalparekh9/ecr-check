...
token = 'KERNEL_LABS_BREEZY_HR_JOBS'


class CRAWLER(object):
    def __init__(self):
        self.baseUrl = 'https://kernel-labs.breezy.hr/'
    ...
    def process_logic(self):
        try:
            url = 'https://kernel-labs.breezy.hr/'
            ...
            if isloaded:
                ...
                if links:
                    for link in links:
                        ...
                        url = str("https://kernel-labs.breezy.hr") + str(link.find('a').get('href'))
                        jobObj['url'] = url
                        isloaded, jobres = self.get_request(url)
                        if isloaded:
                            jobDetail = BeautifulSoup(jobres.text, 'lxml')
                            jobObj['title'] = link.find('h2').text.replace('\n', '').replace('\r', '').strip()
                            jobObj['location'] = link.find('li', {'class': 'location'}).text.replace('\n', '').replace('\r', '').replace("%LABEL_POSITION_TYPE_REMOTE%", "").strip()

...
# Token
token = 'RECRUITEE_CAREER_JOBS_AMPLIFIER'


class CRAWLER(object):
    def __init__(self):
        self.baseUrl = 'https://www.amplifier.com'
    ...
    def process_logic(self):
        try:
            url = f'https://amplifier.recruitee.com/sitemap.xml'
            print(url)
                ...
                if len(links) > 0:
                    for link in links:
                        try:
                            ...
                            if url != "https://amplifier.recruitee.com":
                                isloaded, res = self.get_request(url)
                                if isloaded:
                                    jobObj['url'] = url
                                    data = BeautifulSoup(res.text, "lxml")

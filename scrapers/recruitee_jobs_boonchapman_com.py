...
# Token
token = 'RECRUITEE_CAREER_JOBS_BOONCHAPMAN'


class CRAWLER(object):
    def __init__(self):
        self.baseUrl = 'https://www.boonchapman.com'
    ...
    def process_logic(self):
        try:
            url = f'https://boonchapman.recruitee.com/sitemap.xml'
            print(url)
                ...
                if len(links) > 0:
                    for link in links:
                        try:
                            ...
                            if url != "https://boonchapman.recruitee.com":
                                isloaded, res = self.get_request(url)
                                if isloaded:
                                    jobObj['url'] = url
                                    data = BeautifulSoup(res.text, "lxml")

...
# Token
token = 'RECRUITEE_CAREER_JOBS_ENSEMBLEIQ'


class CRAWLER(object):
    def __init__(self):
        self.baseUrl = 'https://www.ensembleiq.com'
    ...
    def process_logic(self):
        try:
            url = f'https://ensembleiq1.recruitee.com/sitemap.xml'
            print(url)
                ...
                if len(links) > 0:
                    for link in links:
                        try:
                            ...
                            if url != "https://ensembleiq1.recruitee.com":
                                isloaded, res = self.get_request(url)
                                if isloaded:
                                    jobObj['url'] = url
                                    data = BeautifulSoup(res.text, "lxml")

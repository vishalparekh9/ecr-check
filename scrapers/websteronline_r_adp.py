...
# Token
token = 'WEBSTERONLINE_CARRIERS_JOBS'


class CRAWLER(object):
    def __init__(self):
        self.baseUrl = 'https://recruiting.adp.com/srccar/public/nghome.guid?c=1141241&d=WebsterExternalCareerSite'
    ...
    def post_request(self, page):
        try:
            url = 'https://recruiting.adp.com/srccar/public/rest/1/1154907/search/'
            data = '{"filters":[{"name":"state","label":"State"},{"name":"city","label":"City"},{"name":"grp","label":"Functional Group"},{"name":"typeOfFulltime","label":"Full Time/ Part Time"},{"name":"payType","label":"Pay Type"}],"results":{"pageTitle":"Search Results","zeroResultsMessage":"Were sorry but we have no job openings at this time that match your search criteria. Please try another search.","searchFailureMessage":"Oops! Something went wrong.  Search has encountered a problem. Try searching again","resultsFoundLabel":"results found","bookmarkText":"Bookmark This","pageSize":"250","sortOrder":"00001000","shareText":"Share","fields":[{"name":"ptitle","label":"Published Job Title"},{"name":"typeOfFulltime","label":"Full-time"},{"name":"grp","label":"Functional Group"},{"name":"timestampStatusActive","label":"Date Active"}]},"pagefilter":{"page":'+str(page)+'},"rl":"enUS"}'
                ...
                while isdata:
                    ...
                    if isloaded and 'jobs' in res.json():
                        ...
                        for link in res.json()['jobs']:
                            ...
                            url = 'https://recruiting.adp.com/srccar/public/rest/1/' + str(self.csid) + '/job/' + link[
                                'id']
                            jobObj['url'] = link['url']

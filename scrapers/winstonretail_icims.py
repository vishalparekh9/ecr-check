...
# Token
token = 'JOBS_WINSTON_RETAIL_ICIMS'


class CRAWLER(object):
    def __init__(self):
        self.host = 'https://careers-winstonretail.icims.com'
    ...
    def process_logic(self):
        try:
            ...
            page = 0
            while isdata:
                url = self.host + f'/jobs/search?pr={page}&in_iframe=1&mobile=false&width=809&height=500&bga=true&needsRedirect=false&jan1offset=330&jun1offset=330'
                    ...
                    if links:
                        for link in links:
                            ...
                            for job in jsn:
                                ...
                                url = self.host + f'/jobs/{job["idRaw"]}/dispute-specialist/job?mobile=false&width=809&height=500&bga=true&needsRedirect=false&jan1offset=330&jun1offset=330&in_iframe=1'
                                jobObj['url'] = url
                                ...
                                if match:
                                    ...
                                    if jobObj['location'].strip() == '':
                                        try:
                                            jobObj['location'] = soup.find('div', {'class': 'col-xs-6 header left'}).find_all('span')[
                                                    1].text.strip()
                                        except:
